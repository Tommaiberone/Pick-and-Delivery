...
def benvenuto(client, address):
	...
	position = position_in_list(nome_richiedente)

	if (position == -1):
		clientList.append([nome_richiedente, client, address, True, False])
	else:
		clientList[position][3] = True
...
def position_new_client():
	
	i = 0

	for elem in clientList:
		if elem[3] == True and elem[4] == False:
			return 
		i+=1
	return -1
...
def retrieve_from_list():
	...
	nome_richiedente, client, address, is_waiting, already_been_served = clientList[0]
	daRimuovere = 0
	counter = 0
	...
	if (len(clientList) != 1):
		...
		posizione = position_new_client()

		if posizione != -1:
			nome_richiedente= clientList[posizione][0]
			client= clientList[posizione][1]
			address= clientList[posizione][2]
			timeWaited= clientList[posizione][3]
			daRimuovere=posizione
	...
	clientList[daRimuovere][3] = False
	clientList[daRimuovere][4] = True
	updateList()
